...
`incidence_angle` [rad] from the x-axis (`0` = axial/end-on, `π/2` =
...
function solve_general_shell_fluid_filled_coupled(geometry::ProlateShellGeometry,
        ...
        pole_offset::Real = 1e-3, rtol::Real = 1e-5, solve_reports = nothing)
    ...
    return solve_general_shell_fluid_filled_coupled(
        ...
        frequency_hz, incidence_angle; m_max = m_max, rtol = rtol, solve_reports)
end
...
function solve_general_shell_fluid_filled_coupled(shell_mesh::GeneralShellMesh,
    ...
    n_bem = n_eta - 1
    L_s2b = sparse(shell_to_bem_interpolation(n_eta))
    L_b2s = sparse(bem_to_shell_interpolation(n_eta))
    ...
    for m in 0:m_max
        ...
        A = spzeros(ComplexF64, n_total, n_total)
        ...
        # The cavity BEM normal is opposite to the solid's inner-face normal in B_in.
        A[rows, (off_state + 1):(off_state + n_keep)] = interior_density * omega^2 .*
                                                        (L_s2b * ops.B_in)

        rows = (4n_bem + 1):(4n_bem + n_keep)
        # Pressure traction is -p*n on both solid faces.
        ...
        x = _solve_reported(A, b, solve_reports; mode = m, n_eta, rtol)
