...
let
    @time "Spheroidal coupling at angular nodes" @testset "Spheroidal coupling at angular nodes" begin
        boundary = AS.FluidFilled(1.05, 1.03; coupling = :full)
        for body in (AS.Spheroid(0.02, 0.01), AS.Spheroid(0.01, 0.02))
            amplitudes = [AS.scattering_amplitude(AS.modal(body, boundary, 30.0;
                              incidence_angle = angle, m_max = 2, n_max = 4))
                          for angle in (pi / 2 - 1e-5, pi / 2, pi / 2 + 1e-5)]
            @test all(isfinite, real.(amplitudes))
            @test all(isfinite, imag.(amplitudes))
            @test abs(amplitudes[2] - (amplitudes[1] + amplitudes[3]) / 2) <
                  1e-3 * max(abs(amplitudes[2]), 1e-12)
        end
    end
end
