...
@time @testset "Makie visualization: 2D bistatic plots" begin
    a = 0.01
    c_water = 1477.4
    ...
    @time @testset "plot(sol; kind=:bistatic_polar) builds a figure with non-negative radius" begin
        fap = plot(bem_sol; kind = :bistatic_polar, angles = 0:0.2:(2pi))
        @test fap isa Makie.FigureAxisPlot
        @test fap.axis isa PolarAxis
    end

    @time @testset "plot(sol; kind=:bistatic_cartesian) builds a figure" begin
        fap = plot(bem_sol; kind = :bistatic_cartesian, angles = 0:0.2:(2pi))
        @test fap isa Makie.FigureAxisPlot
        @test fap.axis isa Axis
    end

    @time @testset "plot(sol; kind=:bistatic_map) builds a heatmap figure" begin
        fap = plot(bem_sol; kind = :bistatic_map, thetas = 0:0.3:pi, phis = 0:0.3:(2pi))
        @test fap isa Makie.FigureAxisPlot
    end

    @time @testset "show_incidence adds forward/backscatter reference lines" begin
    ...
    @time @testset "incidence markers are derived from the actual incidence direction, not a fixed 0/pi" begin
    ...
    @time @testset "colorrange: default handles a deep null without erroring, explicit override works" begin
        fap = plot(bem_sol; kind = :bistatic_map, thetas = 0:0.3:pi,
            phis = 0:0.3:(2pi), colorrange = (-60.0, -40.0))
        @test fap isa Makie.FigureAxisPlot
    end

    @time @testset "Rotated observation-plane markers" begin
